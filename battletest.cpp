...
void battletest::transfer(account_name from, account_name to, asset quantity, string memo) {
    if (from == _self || to != _self) {
        return;
    }
    eosio_assert(quantity.symbol == S(4,EOS), "battletest only accepts EOS");
    ...
    if (separator_pos != string::npos) {
        ...
        eosio_assert(refer_account_name_str.length() <= 12, "ref account name can only be 12 chars long");
        refer_account = string_to_name(refer_account_name_str.c_str());
        tb_player refer_player_sgt(_self, refer_account);
        //tb_player refer_player_sgt(_self, _self);
    ...
    st_player player = players.get_or_create(_self, default_player);
    ...
    if (player.affiliate_name != 0) {
        ...
        //refer_player_sgt.set(affilicate_player, _self);
    } else {
...
void battletest::withdraw(account_name to) {
    ...
    if (vault > 0) {
        ...
        action(
                ...
                N(eosio.token),
                N(transfer),
                make_tuple(_self, to, vault_asset, string("battletest withdraw"))
        ).send();
    }
}

void battletest::create(time_point_sec start) {
    require_auth(_self);
